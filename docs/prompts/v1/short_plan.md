...
6. Theo độ dài, ngôn ngữ và POV được cấp, chọn số beat vừa sức. Ghi một object `{"language": "...", "pov": "...", "length_guidance": "..."}` trong `outline` để truyền contract xuống Skeleton; các item khác là string beat. **Echo đúng giá trị backend cấp** cho từng `chapter_id`: không tự chọn POV/độ dài, không để field rỗng, không đổi giá trị — backend từ chối output lệch contract. Không thêm field `contract`/`word_count` ở cấp ChapterPlan vì schema không có. Không chuyển cảnh sang nội tâm ngoài POV chỉ để giải thích lore.
